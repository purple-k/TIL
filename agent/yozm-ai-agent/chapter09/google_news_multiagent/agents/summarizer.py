...
class NewsSummarizerAgent:
        ...
        self.prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """당신은 전문 뉴스 요약 전문가입니다.
                    주어진 뉴스를 핵심만 간결하게 2-3 문장으로 요약해주세요.
                    - 사실만을 전달하고 추측은 피하세요
                    - 중요한 숫자나 날짜는 포함하세요
                    - 명확하고 이해하기 쉽게 작성하세요""",
                ),
    ...
    async def summarize_single_news(self, news_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            summary_response = await chain.ainvoke({"title": news_item["title"], "content": content})
            summary = summary_response.content.strip()
            return {**news_item, "ai_summary": summary or content}

        except Exception as e:
            print(f"    [{self.name}] 요약 오류 (Title: {news_item['title']}): {str(e)[:50]}...")
            return {**news_item, "ai_summary": content}
    ...
    async def summarize_news(self, state: NewsState) -> NewsState:
        ...
        state.messages.append(AIMessage(content=f"{len(summarized_news)}개의 뉴스 요약을 완료했습니다."))
