...
from chapter09.google_news_multiagent.utils import convert_gmt_to_kst
...
class RSSCollectorAgent:
    ...
    async def extract_article_url(self, google_news_url: str) -> str | None:
        """
        Stack Overflow 솔루션 활용
        참조: https://stackoverflow.com/questions/79388897/how-to-scrape-google-rssfeed-links/79388987#79388987
        Google News는 JavaScript를 사용하여 페이지를 리다이렉션시키므로 내부 API를 직접 호출하여 우회
        """
        async with httpx.AsyncClient(follow_redirects=True) as client:
            try:
                ...
                api_response = await client.post(
                    f"{GOOGLE_NEWS_BASE_URL}/_/DotsSplashUi/data/batchexecute", headers=headers, data=payload
                )
    ...
    async def parse_entry(self, entry) -> dict[str, str | None]:
        """RSS 피드 항목을 파싱합니다."""
