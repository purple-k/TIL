...
class ResponseFormat(BaseModel):
    ...
    def validate_status(cls, v):
        if v not in ["success", "failure"]:
            raise ValueError('status는 "success" 또는 "failure"이어야 합니다.')
        return v
...
async def content_safety_guardrail(ctx, agent, input_data):
    ...
    return GuardrailFunctionOutput(output_info=safety_check, tripwire_triggered=not safety_check.is_safe)
